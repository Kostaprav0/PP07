...
namespace Бетон2
{
    public class Form1 : Form
    {
        ...
        private void InitializeComponents()
        {
            ...
            mainTableLayout.RowStyles.Add(new RowStyle(SizeType.Percent, 10)); // Заголовок
            mainTableLayout.RowStyles.Add(new RowStyle(SizeType.Percent, 90)); // Контент
            ...
            // Панель контента (меню + рабочая область)
            var contentTableLayout = new TableLayoutPanel();
            contentTableLayout.Dock = DockStyle.Fill;
            contentTableLayout.ColumnCount = 2;
            contentTableLayout.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 20)); // Меню
            contentTableLayout.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 80)); // Рабочая область
        ...
        private void ShowTables(object sender, EventArgs e)
        {
            ...
            // ПЕРЕД созданием TablesControl проверяем подключение к БД
            if (_context == null || _dynamicContext == null)
            {
                ...
                catch (Exception ex)
                {
                    MessageBox.Show($"Не удалось подключиться к базе данных: {ex.Message}\n\nПроверьте настройки подключения.", "Ошибка подключения",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                    ShowSettings(sender, e); // Показываем настройки подключения
                    return;
                }
            }
